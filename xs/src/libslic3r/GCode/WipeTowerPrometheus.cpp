#include "WipeTowerPrometheus.hpp"
...
public:
	...
	Writer& comment_material(WipeTowerPrometheus::material_type material)
	{
		m_gcode += "; material : ";
		switch (material)
		{
		case WipeTowerPrometheus::PVA:
			m_gcode += "#8 (PVA)";
			break;
		case WipeTowerPrometheus::SCAFF:
			m_gcode += "#5 (Scaffold)";
			break;
		case WipeTowerPrometheus::FLEX:
			m_gcode += "#4 (Flex)";
			break;
		default:
			m_gcode += "DEFAULT (PLA)";
	...
	Writer& append(const char *text) { m_gcode += text; return *this; }
        ...
        Writer& sleep(int milliseconds )
        {
                char strvalue[64];
		sprintf(strvalue, "%d", milliseconds);
		m_gcode += std::string("G4 P") + strvalue + "; sleep\n";
		return *this;
        }
...
WipeTowerPrometheus::material_type WipeTowerPrometheus::parse_material(const char *name)
{
...
WipeTower::ToolChangeResult WipeTowerPrometheus::tool_change(int tool, bool last_in_layer, Purpose purpose)
{
...
WipeTower::ToolChangeResult WipeTowerPrometheus::toolchange_Brim(Purpose purpose, bool sideOnly, float y_offset)
{
...
// Ram the hot material out of the melt zone, retract the filament into the cooling tubes and let it cool.
void WipeTowerPrometheus::toolchange_Unload(
	PrusaMultiMaterial::Writer &writer,
	...
	const int 				 new_temperature)
{
	...
	writer.retract(20, 3000).retract(15, 1500);
        writer.sleep(2500);
        writer.retract(140, 3000);
...
void WipeTowerPrometheus::toolchange_Change(
	PrusaMultiMaterial::Writer &writer,
	const int 		new_tool, 
...
void WipeTowerPrometheus::toolchange_Load(
	PrusaMultiMaterial::Writer &writer,
	const box_coordinates  &cleaning_box)
{
	...
	if (std::abs(writer.x() - xr) < std::abs(writer.x() - xl))
		std::swap(xl, xr);
...
// Wipe the newly loaded filament until the end of the assigned wipe area.
void WipeTowerPrometheus::toolchange_Wipe(
	PrusaMultiMaterial::Writer &writer,
	const box_coordinates  &cleaning_box)
{
...
WipeTower::ToolChangeResult WipeTowerPrometheus::finish_layer(Purpose purpose)
{
